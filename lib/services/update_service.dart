...
class UpdateStatus {
  ...
  String get message {
    ...
    return 'You have the latest version (Build $currentBuild)';
  }
}
...
/// tagging. Build numbers are derived from the GitHub Actions run number
/// (always incrementing, immune to git history rewrites). For local/dev builds
/// without a commit SHA, it falls back to comparing build numbers.
class UpdateService {
  ...
  static Future<UpdateStatus> getUpdateStatus() async {
    try {
      final packageInfo = await PackageInfo.fromPlatform();
      final currentBuild = int.tryParse(packageInfo.buildNumber) ?? 0;
      const currentSha = BuildInfo.commitSha;
